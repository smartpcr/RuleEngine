...
namespace Rules.Validations.Persistence
{
    ...
    public class ExportValidationResultToKusto : IExportValidationResultToKusto
    {
        ...
        public async Task<int> ExportToKusto(string runId, CancellationToken cancel)
        {
            ...
            var ruleSet = await ruleSetRepo.GetById(validationJob.RuleSetId);
            var validationRules = await cache.GetOrUpdateAsync(
                $"{nameof(ValidationRule)}-list-{validationJob.RuleSetId}",
                async () => await validationRuleRepo.GetLastModificationTime(
                    $"c.ruleSetId = '{validationJob.RuleSetId}'", cancel),
                async () =>
                {
                    var rules = await validationRuleRepo.Query($"c.ruleSetId = '{validationJob.RuleSetId}'");
                    return rules.ToList();
                },
                cancel);
            ...
            try
            {
                await kustoClient.BulkInsert(nameof(RuleSet), new List<RuleSet> {ruleSet}, IngestMode.InsertNew, "Id", default);
