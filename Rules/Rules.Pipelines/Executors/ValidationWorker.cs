// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ValidationWorker.cs" company="Microsoft Corporation">
...
namespace Rules.Pipelines.Executors
{
    ...
    public class ValidationWorker : BackgroundService
    {
        ...
        private readonly ILogger<ValidationWorker> logger;
        ...
        public ValidationWorker(IServiceProvider serviceProvider, ILoggerFactory loggerFactory)
        {
            logger = loggerFactory.CreateLogger<ValidationWorker>();
                ...
                try
                {
                    ...
                    if (messages.Any())
                    {
                        foreach (var message in messages)
                        {
                            ...
                            try
                            {
                                run = await validator.Validate(validationJob, run, linkedCancellation.Token);
                            }
                            catch (Exception ex)
                            {
